...
This is optional guidance for any maintained HTTP(S) reverse proxy. EditApp
...
`EDITAPP_AUTH_MODE=trusted_proxy` is only for a proxy that authenticates the
user and sets one bounded `X-Forwarded-User` value. Set
`EDITAPP_TRUSTED_PROXY_CIDRS` to only the proxy's immediate source CIDR(s).
...
For a separate browser origin, configure `EDITAPP_ALLOWED_ORIGINS` with exact
HTTP(S) origins. CORS is deny-by-default; wildcard origins are not supported.
Use `EDITAPP_AUTH_MODE=bearer` when the proxy does not establish a user
