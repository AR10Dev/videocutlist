...
These are optional ways to reach VideoCutlist. The application only needs a
...
Set `VIDEOCUTLIST_PUBLIC_BASE_URL` to the URL given to browsers. If the browser is
served from a different origin, set `VIDEOCUTLIST_ALLOWED_ORIGINS` to that exact
...
```text
VIDEOCUTLIST_LISTEN_ADDRESS=192.0.2.10
VIDEOCUTLIST_PORT=8787
VIDEOCUTLIST_PUBLIC_BASE_URL=http://192.0.2.10:8787
...
Keep VideoCutlist on loopback and use the [Tailscale example](tailscale/README.md)
...
that reachable URL as `VIDEOCUTLIST_PUBLIC_BASE_URL`; protect the tunnel endpoint
with WireGuard peer policy and application authentication as appropriate.

## SSH tunnel (optional)

Leave VideoCutlist on loopback and forward a local browser port through SSH:

```bash
ssh -N -L 8787:127.0.0.1:8787 editor@videocutlist-host
```
...
Keep VideoCutlist loopback-only and place a maintained TLS reverse proxy in front
of it. Use a public `https://` value for `VIDEOCUTLIST_PUBLIC_BASE_URL`, an exact
browser origin in `VIDEOCUTLIST_ALLOWED_ORIGINS` when cross-origin, and bearer or
trusted-proxy authentication. Do not expose the raw VideoCutlist listener directly
