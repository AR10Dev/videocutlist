...
[[ -n $caps ]] || {
  echo "VIDEOCUTLIST_TAILSCALE_APP_CAPS is required" >&2
  exit 2
}
case $caps in
*[!A-Za-z0-9./,:_-]*)
  echo "VIDEOCUTLIST_TAILSCALE_APP_CAPS has invalid characters" >&2
  exit 2
  ;;
esac
...
systemctl is-active --quiet videocutlist || {
  echo "videocutlist is not active" >&2
  exit 1
}
...
printf '%s\n' "$serve" | grep -F '127.0.0.1:8787' >/dev/null || {
  echo "Serve is not proxying 127.0.0.1:8787" >&2
  exit 1
}
