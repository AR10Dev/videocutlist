...
async function addSelectedMediaToProject(page: Page) {
  await page.waitForTimeout(0);
...
test("task tabs expose selection, association, and keyboard navigation", async ({ page }) => {
  ...
  await detection.press("ArrowRight");
  ...
  await exportTab.press("ArrowLeft");
  await expect(detection).toBeFocused();
  await expect(detection).toHaveAttribute("aria-selected", "true");
  await expect(page.getByRole("tabpanel", { name: "Auto-detect" })).toBeVisible();
});
...
test("edits independent project items and submits a durable batch", async ({ page }) => {
  ...
  expect(exportedItems.find((item) => item.mediaId === secondMedia.id)?.exportOptions.mode).toBe(
    "merge",
  );
...
test("loads timeline assets for the visible range and keeps editing available on media changes", async ({
  ...
  await expect(page.getByText("No segments yet.", { exact: true })).toBeVisible();
...
test("save reports an optimistic revision conflict", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Reload remote project" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Save local work as new project" })).toBeVisible();
});
...
test("covers the responsive workspace and keyboard editing workflow", async ({ page }) => {
  ...
  await expect(page.locator(".project-sidebar-header strong")).toHaveText("Untitled project");
  await expect(projectStatus).toContainText("Not saved yet");
  ...
  const exportTab = page.getByRole("tab", { name: "Export", exact: true });
  const detectionTab = page.getByRole("tab", { name: "Auto-detect", exact: true });
  ...
  await exportTab.focus();
  await page.keyboard.press("Enter");
  const startExport = page.getByRole("button", { name: "Create clips" });
...
test("keeps the preview stable and exposes familiar playback controls", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Previous preview step" }).click();
...
test("renders separate timeline lanes and seeks through their shared interaction area", async ({
  ...
  await page.getByRole("button", { name: "Set out" }).click();
  const outMarker = page.locator(".cut-handle-end");
  ...
  const originalOut = Number(await outMarker.getAttribute("aria-valuenow"));
  await page.mouse.move(currentBounds!.x + (currentBounds!.width * 4) / 5, currentBounds!.y + 8, {
    steps: 3,
  });
  await page.mouse.up();
  await expect
    .poll(async () => Number(await outMarker.getAttribute("aria-valuenow")))
    .toBeGreaterThan(originalOut);
  await expect(page.locator("#timeline-description")).toContainText(/Out marker 00:08\.0/);

  await page.waitForTimeout(120);
  const inMarker = page.locator(".cut-handle-start");
  ...
  const handleMetrics = await inMarker.evaluate((element) => ({
    visualWidth: Number.parseFloat(getComputedStyle(element).width),
    hitWidth: Number.parseFloat(getComputedStyle(element, "::before").width),
  }));
  expect(handleMetrics.visualWidth).toBe(12);
  expect(handleMetrics.hitWidth).toBeGreaterThanOrEqual(12);
  const activeSegmentBounds = await page.locator(".timeline-segment.is-active").boundingBox();
  expect(activeSegmentBounds).not.toBeNull();
  expect(handleMetrics.hitWidth).toBeLessThanOrEqual(activeSegmentBounds!.width / 2 + 1);
  ...
  await timeline.click({
    position: { x: (settledBounds!.width * 9) / 10, y: settledBounds!.height / 2 },
  });
  ...
  await expect(timelineSegments.nth(0)).toHaveAttribute("aria-pressed", "false");
  await expect(timelineSegments.nth(1)).toHaveAttribute("aria-pressed", "true");
});
...
test("covers theme contrast and selected task control states", async ({ page }) => {
  await page.goto("/");
  const cutsTab = page.getByRole("tab", { name: "Cuts", exact: true });
  await expect(cutsTab).toHaveAttribute("aria-selected", "true");
  ...
  await page.getByRole("button", { name: "Show editing tools" }).click();
  await expect(cutsTab).toHaveCSS("font-weight", "700");
  await cutsTab.focus();
  await expect(cutsTab).toBeFocused();
...
test("keeps motion reduced and overflow local at a narrow viewport", async ({ page }) => {
  ...
  await expect(page.locator(".cut-row[data-segment-id]")).toHaveCount(1);
  await expect(
    page.getByRole("listitem", { name: /Segment 001/ }).getByRole("button", {
      name: "Remove cut 1",
    }),
  ).toBeVisible();
