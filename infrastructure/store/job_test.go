...
func TestJobStateTransitionsAndRecovery(t *testing.T) {
	...
	if _, err := jobs.Create(ctx, store.ExportJob{ID: "j_000000000101", ProjectID: "project", ProjectRevision: 1, RequestJSON: `{}`}); err != nil {
		t.Fatal(err)
	}
	for _, id := range []string{"j_000000000102", "j_000000000103"} {
		if _, err := jobs.Create(ctx, store.ExportJob{ID: id, ProjectID: "project", ProjectRevision: 1, RequestJSON: `{}`}); err != nil {
			t.Fatal(err)
		}
	...
	queued, err := jobs.Get(ctx, "j_000000000101")
	if err != nil || queued.State != store.JobQueued {
		t.Fatalf("queued job after recovery = %#v, %v", queued, err)
	}
	job, err := jobs.Get(ctx, "j_000000000102")
	if err != nil || job.State != store.JobFailed || job.ErrorCode.String != "interrupted_by_restart" {
		t.Fatalf("recovered job = %#v, %v", job, err)
	}
	if _, err := jobs.Start(ctx, "j_000000000102"); !errors.Is(err, store.ErrJobState) {
		t.Fatalf("terminal job restarted: %v", err)
	}
	if _, err := jobs.Create(ctx, store.ExportJob{ID: "j_000000000104", ProjectID: "project", ProjectRevision: 1, RequestJSON: `{}`}); err != nil {
		t.Fatal(err)
	}
	if _, err := jobs.Start(ctx, "j_000000000104"); err != nil {
		t.Fatal(err)
	}
	completed, err := jobs.Succeed(ctx, "j_000000000104", `{"outputName":"export.mkv","retainUntil":"2026-01-01T00:00:00Z"}`)
...
func TestJobLookupIgnoresCompatibilityOwner(t *testing.T) {
	...
	ctx := context.Background()
	if _, err := jobs.Create(ctx, store.ExportJob{ID: "j_000000000105", ProjectID: "project", ProjectRevision: 1, RequestJSON: `{}`}); err != nil {
		t.Fatal(err)
	}
	if _, err := jobs.Get(ctx, "j_000000000105"); err != nil {
