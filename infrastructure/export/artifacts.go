...
func (s *ArtifactStore) Reconcile(ctx context.Context, jobs *store.JobsStore, ffprobePath string, destinations []Destination) error {
	if jobs == nil {
...
func VerifyArtifact(ctx context.Context, ffprobePath, path string) error {
	...
	metadata, err := (probe.Client{Path: ffprobePath}).Probe(ctx, path)
