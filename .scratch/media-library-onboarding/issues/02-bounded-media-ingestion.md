...
Implementation evidence: `application/services.go` now restores a usable library state on context cancellation and expires terminal import entries after the documented one-minute retention window. Focused tests in `application/services_test.go` cover cancellation during catalog refresh and terminal-job expiry. Validation: `gofmt -w application/services.go application/services_test.go`; `go test ./application` (passed, 15 tests); `git diff --check` (passed).
