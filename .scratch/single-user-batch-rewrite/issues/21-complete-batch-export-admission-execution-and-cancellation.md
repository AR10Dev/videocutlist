...
- [ ] Batch export admission uses one atomic scheduler-backed capacity check and child-job insertion.
- [ ] Export execution decodes the stored snapshot, validates the current source fingerprint, and fails the child with `source_changed` when it differs or is unavailable.
- [ ] Batch cancellation terminally cancels queued children and requests cancellation of running execution contexts.
- [ ] Regression tests cover capacity rejection, changed source execution failure, and running batch-child cancellation.
