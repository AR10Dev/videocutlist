...
- Publication uses atomic no-replace links and retries with a fresh generated name instead of overwriting a concurrently created file.
- Recovery scans configured download/archive roots and source-adjacent media roots, never follows symlinked directories, validates the exact open descriptor, and only touches basenames listed by a valid job manifest.
