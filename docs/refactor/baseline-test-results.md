...
| `npm --prefix web run test:e2e` | initial 127; provisioned 1 | Initial Playwright binary was absent. After locked install and scoped loopback execution, 1 of 2 tests passed; the primary browser flow failed waiting for `Project saved` at `web/playwright/segment-selection.spec.ts:130`. |
...
| Playwright | `npm --prefix web run test:e2e` | Failed after provisioned execution: 1 passed, 1 failed waiting for `Project saved`. |
...
The provisioned baseline passes all Go and non-browser web gates, but the browser suite has one functional failure. The initial cache/dependency/loopback failures are environmental and were resolved only for baseline verification; no repair was made. Tailscale remains absent, as required for the transport-neutral refactor verification.
