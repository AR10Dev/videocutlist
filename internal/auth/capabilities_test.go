...
func TestPrincipalAllows(t *testing.T) {
	principal := Principal{Capabilities: []string{"preview", "export:p_allowed"}}
	...
	if !(Principal{Capabilities: []string{"*"}}).Allows("anything", "anywhere") {
		t.Fatal("wildcard capability was rejected")
	}
}

func TestBuiltinAuthenticatorModes(t *testing.T) {
	none, err := New(Config{Mode: "none"})
	if err != nil {
		t.Fatal(err)
	}
	...
	bearer, err := New(Config{Mode: "bearer", BearerToken: "alpha beta"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTrustedProxyAuthenticatorUsesOnlyContext(t *testing.T) {
	authenticator, err := New(Config{Mode: "trusted_proxy"})
	if err != nil {
		t.Fatal(err)
	}
	...
	handler, err := httpx.TrustedProxy([]string{"127.0.0.0/8"}, http.HandlerFunc(func(_ http.ResponseWriter, request *http.Request) {
