...
func TestBuildPreviewArgsUsesInheritedFD(t *testing.T) {
	...
	defer source.Close()
	args, err := BuildPreviewArgs(contracts.PreviewSpec{Source: source, DurationMS: 1, Width: 1280, Height: 720, FPS: 30, Audio: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStartHonorsCancellation(t *testing.T) {
	...
	running, err := (Runner{Path: os.Args[0]}).Start(ctx, contracts.PreviewSpec{Source: source, DurationMS: 1, Width: 16, Height: 16, FPS: 1})
	if err != nil {
		t.Fatal(err)
	}
	cancel()
