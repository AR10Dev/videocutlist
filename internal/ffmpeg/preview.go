...
func BuildPreviewArgs(spec contracts.PreviewSpec) ([]string, error) {
	if spec.Source == nil || spec.StartMS < 0 || spec.DurationMS < 1 || spec.Width < 1 || spec.Height < 1 || spec.Width > 1280 || spec.Height > 720 || spec.FPS < 1 || spec.FPS > 30 {
...
// Start implements contracts.PreviewRunner.
func (r Runner) Start(ctx context.Context, spec contracts.PreviewSpec) (*contracts.RunningPreview, error) {
	...
	cmd.ExtraFiles = []*os.File{spec.Source}
	...
	return &contracts.RunningPreview{Stdout: &timedReadCloser{ReadCloser: stdout, state: state}, PID: cmd.Process.Pid, Wait: wait}, nil
}
