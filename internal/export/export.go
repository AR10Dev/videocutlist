...
func (s Service) Run(ctx context.Context, source *os.File, document projects.Document, request Request) (Result, error) {
	if source == nil {
...
func (s Service) copySegment(ctx context.Context, source *os.File, segment projects.Segment, output string) error {
...
func hasPotentiallyInexactCut(segments []projects.Segment) bool {
	for _, segment := range segments {
