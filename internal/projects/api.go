// Package projects contains project and job use cases plus their consumer-owned ports.
package projects
...
type Job struct {
	...
	MediaLabel      string                       `json:"mediaLabel,omitempty"`
	...
	Kind            model.DetectionKind          `json:"kind,omitempty"`
...
type ProjectListService interface {
	List(context.Context, string, int) (ProjectPage, error)
}
...
func NormalizePreview(mediaID string, durationMS, centerMS int64, mute bool, cfg model.WindowConfig) (PreviewSpec, error) {
	window, err := model.Normalize(centerMS, durationMS, cfg)
	if err != nil {
