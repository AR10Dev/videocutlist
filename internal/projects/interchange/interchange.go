...
func ExportCSV(segments []model.Segment) ([]byte, error) {
	if _, err := validate(segments, 0); err != nil {
		return nil, err
	}
	var b bytes.Buffer
	w := csv.NewWriter(&b)
...
func ExportChapters(segments []model.Segment) ([]byte, error) {
	if _, err := validate(segments, 0); err != nil {
		return nil, err
	}
...
func validate(segments []model.Segment, durationMS int64) ([]model.Segment, error) {
