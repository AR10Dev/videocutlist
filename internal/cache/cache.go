...
var (
	ErrMiss      = errors.New("preview cache miss")
...
func (f ValidatorFunc) Validate(ctx context.Context, path string) error { return f(ctx, path) }
...
func Key(spec contracts.PreviewSpec) string {
	payload := struct {
		V     int `json:"v"`
		Media struct {
			ID        string `json:"id"`
	...
	payload.Preview.Width, payload.Preview.Height, payload.Preview.FPS = spec.Width, spec.Height, spec.FPS
	payload.Preview.Audio = spec.Audio
	payload.Preview.VideoCodec, payload.Preview.AudioCodec, payload.Preview.Mux = "h264", "aac", "fmp4"
	payload.Encoder.Profile = spec.Encoder
...
func (p *Partial) Commit(ctx context.Context, validator Validator) error {
	...
	p.once.Do(func() {
		...
		if err := validator.Validate(ctx, p.path); err != nil {
